#!/usr/bin/env bash
# =====================================================================
# load-images.sh — 从外部 images/ 目录加载离线镜像（252 服务器用）
#
# 扫描 ${POCKET_IMAGE_DIR} 下所有 *.tar / *.tar.gz，逐个 docker load。
# 252 上一般以 root 运行：sudo ./deploy/bin/load-images.sh
#
# 用法：
#   sudo ./deploy/bin/load-images.sh              # 全部加载
#   sudo ./deploy/bin/load-images.sh --latest     # 只加载每个镜像最新一份
# =====================================================================

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck disable=SC1091
source "${SCRIPT_DIR}/env.sh"

LATEST_ONLY=false
while [[ $# -gt 0 ]]; do
  case "$1" in
    --latest) LATEST_ONLY=true; shift ;;
    --help) echo "用法: $0 [--latest]"; exit 0 ;;
    *) echo "未知参数: $1"; exit 1 ;;
  esac
done

if [[ ! -d "${POCKET_IMAGE_DIR}" ]]; then
  echo "❌ images 目录不存在: ${POCKET_IMAGE_DIR}" >&2
  exit 1
fi

shopt -s nullglob
TARS=("${POCKET_IMAGE_DIR}"/*.tar "${POCKET_IMAGE_DIR}"/*.tar.gz)
shopt -u nullglob

if [[ ${#TARS[@]} -eq 0 ]]; then
  echo "❌ ${POCKET_IMAGE_DIR} 下没有 *.tar / *.tar.gz" >&2
  exit 1
fi

# --latest：每个镜像名只保留时间戳最新的一份
if [[ "${LATEST_ONLY}" == true ]]; then
  LATEST_TARS=()
  for prefix in "opencode-pocket-frontend" "opencode-pocket"; do
    # 注意顺序：长名前缀在前，避免 frontend 被 pocket 匹配走
    best=""
    for f in "${TARS[@]}"; do
      base="$(basename "${f}")"
      [[ "${base}" == "${prefix}"_* ]] || continue
      [[ -z "${best}" || "${f}" > "${best}" ]] && best="${f}"
    done
    [[ -n "${best}" ]] && LATEST_TARS+=("${best}")
  done
  TARS=("${LATEST_TARS[@]}")
fi

echo "━━━ load-images: ${POCKET_IMAGE_DIR} ━━━"
for f in "${TARS[@]}"; do
  echo "▶ docker load < $(basename "${f}")"
  if [[ "${f}" == *.gz ]]; then
    gunzip -c "${f}" | docker load
  else
    docker load -i "${f}"
  fi
done

echo "✅ 加载完成；验证："
docker images | grep -E "opencode-pocket" || true
echo "   下一步: sudo ./deploy/bin/deploy-252.sh"
