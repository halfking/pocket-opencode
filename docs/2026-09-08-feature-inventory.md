...
> **2026-09-23 注**：本文为 2026-09-08 盘点快照（前端 25 域 + 后端 47 模块）。此后新增 `flashcards` / `more` / `study` 等前端域（09-23 融合重构 Phase 1-2）与 `flashcards` 后端模块（09-19），当前实数 = 前端 30 域 + 后端 48 模块；新增域明细待后续增补，正文各表保留 09-08 时点内容。
