...
| Mobile frontend (Vue 3 + Capacitor) | `docs/2026-08-27-mobile-ux-design-v2.md`（产品/交互现行规范）, `docs/2026-08-27-p1-contracts-frozen.md`（P1 契约）, `docs/2026-07-02-ui-ux-design-system.md`（设计系统） | `implemented (unverified)` — P0 已落地（分诊条/健康信号/内联审批/长按控制/本地通知，真机未验证）；**P1 已落地**（会话工作台轮次化：状态条/轮次时间线/详情抽屉/旧详情 301 兜底；输入系统：SessionComposer/指令模板/`local_drafts` 草稿持久化；均待真机验证）；P2-P3 未开工 | P0 健康度模型 `contract-tested`（8 用例，绿色日志 `test-evidence/P0-mobile-ux/gate-run-2026-08-27.log`）；P1 事件消费纯逻辑 `contract-tested`（`frontend/src/features/sessions/__tests__/useSessionEvents.test.mjs` 16 用例 + `useSessionDrafts.test.mjs` 15 用例 + `draftStore.test.mjs` 4 用例 + TS 事件 fixture 6 用例，绿色日志 `test-evidence/P1-mobile-ux/gate-run-2026-08-27.log`）；UI 集成 `source-inspected` | `NAVIGATION_ARCHITECTURE.md`, `MOBILE_ARCHITECTURE_V2.md`, `docs/2026-07-03-mobile-interaction-optimization.md`（均 superseded） | — | `frontend/src/features/tasks/{health.ts,useInstanceApprovals.ts,TasksView.vue}`, `frontend/src/composables/useApprovalAlerts.ts`, `frontend/src/features/sessions/{SessionConversationView.vue(637 行收敛),SessionStatusBar.vue,RoundTimeline.vue,SessionDetailDrawer.vue,SessionComposer.vue,useSessionEvents.ts,useSessionDrafts.ts}`, `frontend/src/native/{draftStore.ts,schema.ts(local_drafts)}`, `frontend/src/services/sessionEvents.ts`, `frontend/src/composables/useBreakpoint.ts`。停止通道：`api.interruptSession` → `POST /api/mobile/sessions/:id/interrupt`（plugin_hub `session.stop` 信封协议缺口见 EVIDENCE-LEDGER 备注）。`task.health` 前端消费 P2（TasksView 仍用 P0 近似） |
| pocketd 会话事件层（`session.activity`/`round.completed`/`task.health` + snapshot） | `docs/2026-08-27-p1-contracts-frozen.md` §2/§3（冻结契约）, `docs/2026-08-27-mobile-ux-design-v2.md` §5.1/§5.2 | `contract-tested` | `contract-tested` | — | — | `backend/internal/opencode/session_event_broadcaster.go`（+`_test.go` Go fixture）, `backend/internal/server/mobile_events_handler.go`（+`_test.go` 端点行为）, `frontend/src/services/sessionEvents.ts`（+`__tests__/sessionEvents.test.mjs` TS fixture，Go↔TS 双向锁定）；绿色日志 `test-evidence/P1-mobile-ux/gate-run-2026-08-27.log`（§6 `go test ./...` 全 ok + §7 契约 `-race` 复跑）。EVIDENCE-LEDGER「pocketd 会话事件层」行 |
