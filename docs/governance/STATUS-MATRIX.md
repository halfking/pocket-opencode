...
> **Last updated**: 2026-09-08（会议工作台 as-built：`docs/2026-09-08-meetings-studio.md`；列表顶栏筛选 + 详情归档/删除/ACC）  
> 上一版：2026-08-28（P3 反馈轮：图标字形修复 + 深色对比 + AI 网关格式/常用模型；P2 详情见 EVIDENCE-LEDGER）
...
|---|---|---|---|---|---|---|
| OpenCode adapter (legacy `backend/internal/adapter/opencode_http.go`) | `docs/opencode-task-management-architecture.md`（已删除）, `docs/archive/2026-07/OPENCODE_*.md`（原 root + `docs/` 位置，已归档） | `implemented (unverified)` | `source-inspected` | — | `docs/新架构v1/02-modules/redclaw-integration.md`, `docs/新架构v1/04-contracts/pocket-zag-incremental.md` | `backend/internal/adapter/opencode_http.go`, `backend/internal/zagclient/` (reserved) |
...
| RedClaw adapter (`backend/internal/redclaw/legacy`, `facade`, `mcp`) | `docs/新架构v1/02-modules/redclaw-integration.md`, `docs/archive/2026-08/优化v4/07-AI编排与智能体控制面.md` | `implemented (unverified)` (legacy) / `contract-tested` (facade) / `implemented (unverified)` (mcp) | `source-inspected` | — | `docs/新架构v1/04-contracts/pocket-adapter-matrix.md` | `backend/internal/redclaw/`, `docs/新架构v1/04-contracts/_status.md` |
| ACC MCP (`backend/internal/host_gateway` ACC connector) | `docs/ACP_INTEGRATION.md`（已删除）, `docs/新架构v1/02-modules/redclaw-integration.md` | `implemented (unverified)` | `source-inspected` | — | — | `backend/internal/host_gateway/` (MCP), `backend/internal/zagclient/` (pending ZAG ACP) |
...
| IDE connectors (VS Code / Cursor) | `docs/新架构v1/02-modules/ide-control.md`, `docs/superpowers/plans/*redclaw*`（已删除） | `planned` | `claimed (unverified)` | — | — | (none — design only) |
...
| WebSocket hub (`pocketd` WS / SSE) | `docs/WEBSOCKET_REALTIME_UPDATE.md`（已删除）, `docs/新架构v1/01-architecture/数据流与协议.md` | `implemented (unverified)` | `source-inspected` | — | — | `backend/internal/server/ws_*.go`, `docs/superpowers/specs/2026-08-17-task-approval-projection-design.md`（已删除） |
...
- The `docs/优化v4/` plan set (`01-现状审计与差距.md` … `16-PR2-ADR-api-v4-延后.md`, archived at `docs/archive/2026-08/优化v4/` since 2026-09-14), plus `docs/优化v4/reports/` (kept in place)
