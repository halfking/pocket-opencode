...
import { initSqliteWeb } from "./native/sqlite-web-init"
...
async function bootstrap() {
  // Web 端尽量先初始化 jeep-sqlite；失败不阻塞挂载（本地库功能降级）。
  // 15s 超时兜底：IndexedDB 被禁用 / 私密模式等场景下 sql-wasm 初始化可能挂起。
  try {
    await Promise.race([
      initSqliteWeb(),
      new Promise((_, reject) =>
        setTimeout(() => reject(new Error("sqlite-web init timeout")), 15000),
      ),
    ])
  } catch (err) {
    console.error("[bootstrap] sqlite-web init failed:", err)
  }
